...
#parametros do modelo probabilistico
K1=1
b=0.75

#parametro para medida F
bTerm=1

#parametro do método Rocchio
alpha=1
beta=0.75
gamma=0.15
...
def getTF(terms, documents, query):
	print "\n*************\n", terms
	tfMatrix=[]
...
def getIDF(terms, documents, query):
	idfMatrix=[]
	documents.append(*query)
	ndocs=len(documents)
	for word in terms:
...
def tfIdf(terms, documents, query):
	tfMatrix = getTF(terms, documents, query)
	#print "\n===========\ntf-matrix\n", np.matrix(tfMatrix)
	idfMatrix = getIDF(terms, documents, query)
	#print "\n===========\nidf-matrix\n", np.matrix(idfMatrix).transpose()
	tfidf=getTfIdf(tfMatrix,idfMatrix)
	#print "\n===========\ntfidf-matrix\n", tfidf
	return tfidf
...
def getRecall(relevant, recovered):
...
def getPrecision(relevant, recovered):   
	inter = [x for x in relevant if x-1 in recovered]
	l=float(len(inter))
...
def fMeasure(j, ranking):
	result = 0
	Rj=getRecall(R1, ranking)
	Pj=getPrecision(R1, ranking)
	numerator=(1+(bTerm*bTerm))*Pj*Rj
	denominator=((bTerm*bTerm)*Pj)+Rj
...
def interpolatedPrecision(recall, precision, recovered):
	...
	for i in range(len(recovered)):
		if recovered[i]+1 in R1:
			...
			r=getRecall(R1, rankslice)
			p=getPrecision(R1, rankslice)
			levels.append([r,p])
...
def MAP(query, relevant, recovered):
	...
	for i in relevant:
		if i-1 in recovered:
			soma+=getPrecision(R1, recovered[:recovered.index(i-1)+1])
	result = soma/n
	return result

def rocchio(Dn, Cr, Dr):
	print "START"
	Cr=[x-1 for x in Cr]
	print Dn, Cr, Dr
	Nr=len(Dr)
...
def VectorSpaceModelImplementation(docList, separators, stopwords, query):
	...
	similarities = getRankingsTFIDF(TfIdfMatrix)
	...
	recall=getRecall(R1, ranking)    
	precision=getPrecision(R1, ranking)
	...
	#começo do método de Rocchio
	rocchio([f for f in tRanking if f+1 not in R1], list(R1), [f for f in tRanking if f+1 in R1])
...
def simbm25(document, incidence, wordList, query, avgDocLength, niList):
	...
	for word in wordList:
		if word in query[0] and word in document:
			...
			similarity+=bTerm*logTerm
			#print word, "\n", bTerm, "\n", logTerm, "\n", similarity
		count+=1
	return similarity

def getRankingsProbabilistic(documents,query,docIncidence, wordList):
	finalRankings =[]
	docLengthList=[]
	for doc in documents:
...
def probabilisticModelImplementation(docList, separators, stopwords, query):
	...
	rankings = getRankingsProbabilistic(finalDocList,finalQuery,docIncidenceMatrix,wordList)  
